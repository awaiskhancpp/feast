...
export function TransactionDetailsPanel({ transaction }: TransactionDetailsPanelProps) {
  if (!transaction) {
    return (
      <div className="h-full rounded-2xl border border-gray-100 bg-white p-4 shadow-sm">
        <h4 className="text-base font-semibold text-gray-900">Detail Orders</h4>
  ...
  return (
    <aside className="flex h-full flex-col rounded-2xl border border-gray-100 bg-white p-4 shadow-sm">
      <div className="mb-4">
        <h4 className="text-lg font-semibold text-gray-900">Detail Orders</h4>
      </div>
        ...
        {transaction.details.map((line) => (
          ...
          >
            ...
            <div className="min-w-0">
              ...
              <p className="mt-0.5 text-xs text-gray-500">Note : {line.note}</p>
...
function Row({ label, value, strong }: { label: string; value: string; strong?: boolean }) {
  return (
    <div
      className={`flex items-center justify-between ${strong ? 'text-base font-bold text-gray-900' : ''}`}
    >
      <span className={strong ? 'text-gray-600' : 'text-gray-400'}>{label}</span>
      <span>{value}</span>
    </div>
  )
}
